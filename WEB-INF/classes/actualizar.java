...
public class actualizar extends HttpServlet
{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			pst = conexion.prepareStatement("update alumno set matricula=?, carrera=?, fecha_registro=?, semestre=?, tutor=? where id_persona="+id);
			pst.setString(1, ncontrol);
			pst.setString(2, carrera);

			if(fechareg.contains("-"))
				pst.setDate(3, Date.valueOf(LocalDate.parse(fechareg, DateTimeFormatter.ofPattern("dd-MM-uuuu"))));
			else
				pst.setDate(3, Date.valueOf(LocalDate.parse(fechareg, DateTimeFormatter.ofPattern("dd/MM/uuuu"))));

			pst.setString(4, semestre);
			pst.setString(5, tuto);
			pst.executeUpdate();
			out.println("Actualización de alumno exitosa");
